...
Composer text is saved through the [Session Controller draft API](../../api/session-controller/README.md), outside the transcript. A browser-local text mirror provides immediate reload recovery and persists whether its text is unsaved. Unsaved local text, including an empty clear, or edits made while the Host read is pending take precedence over that read. Otherwise the composer adopts the Host draft without submitting it, even when its local mirror is nonempty. Host acknowledgement marks only the matching local text as saved; older browser records without the saved-state flag count as unsaved until synchronized. Host writes are serialized and coalesced while another write is in flight; scope disposal detaches observation and disposes the input shell immediately before waiting for accepted writes. Storage failures leave the editor intact and show a localized error. This persistence covers text only, not attachment objects or structured editor references, and does not provide live synchronization between tabs.
