...
export const en = {
  ...
  settingsUnavailableRemote: 'Settings live on the machine that runs the harness, and this page is not on it. Open the GUI there, or forward that machine\'s loopback port over SSH, to change models and credentials.',
...
export const zh: { [Key in keyof typeof en]: string } = {
  ...
  settingsUnavailableRemote: '设置保存在运行 Harness 的机器上，而当前页面不在该机器上。请在该机器上打开界面，或通过 SSH 转发其回环端口，以修改模型与凭据。',
