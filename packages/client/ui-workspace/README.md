...
Use the sidebar to browse Workspaces and their Sessions, reorder them, and start new ones; use the picker in the Session Intent hero to choose a Workspace for a new session. An open Workspace shows five non-blank Sessions by default and keeps the selected blank **New Session** as one provisional extra row until its first prompt. **Show more** reveals the hidden remainder; closing and reopening the Workspace restores this folded projection. Opening a Session reopens the Workspace that owns it, including from a URL that names one, and folding that Workspace again is preserved; a folded Workspace holding the open Session keeps the business-color folder plus a 2px leading bar so its row stays identifiable. A document whose URL carries a well-formed `?session=` starts focused: the sidebar is collapsed while this plugin activates, before the frame's first render, so the rail never animates in from a visible panel, and the user's next toggle stands. Whether that id is listed is settled later and does not change the sidebar; an unusable link still reports itself through the overlay. Without the parameter, or with a malformed one, the frame keeps its default width.
