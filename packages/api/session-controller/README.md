...
`session/getDraft({ request: { sessionId } })` reads saved composer text; `session/setDraft({ request: { sessionId, text } })` replaces it and acknowledges only after durability. Both return `{ text }` and validate the Session through a cold query without resuming an Agent or appending transcript events. Empty text deletes the record; a missing draft reads as empty text. Callers serialize dependent edits because concurrent writes are last-write-wins. The required `storageDomain` service stores text-only records in the versioned `session_composer_drafts` domain with per-record layout; controller disposal drains writes and closes the domain. Drafts are not copied by Session forks.
