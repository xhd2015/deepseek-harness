...
当已挂载的执行器约束命令（例如 `dsh-bash-sandbox`）时，被阻止的文件操作会报告为 `[sandbox: file access denied under <mode> mode]`——这是策略拒绝，不是命令失败。只有携带 `[sandbox: escalation available` 标记的结果才授权重试：参数错误不是拒绝。出现该标记后，模型可以在同一轮次中用 `sandbox_permissions`（满足需要的最窄更宽模式）与一句 `justification` 重试完全相同的命令一次；该重试引发的审批提示就是用户同意的方式。升权绝不能预先推测：没有该标记的请求不构成拒绝，而无法加宽调用模式的请求会被忽略——命令在常驻模式下运行，结果会说明这一点。可能加宽该调用、却缺少一句理由的请求会被拒绝。被拒绝的升权对该命令即为最终结果。
