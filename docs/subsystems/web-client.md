...
The Session Controller's `session/getDraft` and `session/setDraft` methods read and replace durable composer text without activating an Agent or adding transcript events. [Session Controller](../../packages/api/session-controller/README.md) owns the draft API and storage rules; [UI Conversation](../../packages/client/ui-conversation/README.md) owns browser recovery and serialized synchronization.
