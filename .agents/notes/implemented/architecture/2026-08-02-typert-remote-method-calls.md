...
`@deepseek-ai/dsh-api-gateway`, located at `packages/api/gateway`, provides two symmetric faces: its default entry provides Host `ctx.typertGateway`, while its `/client` entry provides consumer-side `ctx.remote`. Each side consumes a locally generated `InvocationDescriptor` from the same model; descriptors are not sent over the wire. Connection retains its HTTP `/api` RPC channel, while browser Gateway unary calls and streams use the Gateway mux carrier; the business calling interface remains independent of the selected carrier.
