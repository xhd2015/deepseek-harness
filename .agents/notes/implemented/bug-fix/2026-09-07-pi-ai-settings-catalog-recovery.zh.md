...
`LlmConfigurableProvider.error` 优先为提供方行携带首个模型诊断，无模型诊断时返回路由错误，`LlmConfigurableProvider.modelErrors` 则按模型 ID 携带每一条拒绝原因。提供方构造失败不会覆盖已收集的模型诊断，从而保留缺少协议时的具体修复提示。可配置提供方目录发布诊断变化，修复配置会刷新浏览器，无需重新注册适配器。错误模型 ID 保留在设置中，模型选择器将其与服务可用条目并列列出，置灰并携带原因（[只拒绝单个模型](../feature/2026-09-18-refuse-one-model-not-the-namespace.zh.md)）。模型设置页显示诊断；存在对应模型行时显示在该行上，并保留编辑、删除控件。两个添加操作都要求其所属 settings 命名空间存在；普通添加菜单会过滤不可用的命名空间。
...
依赖升级产生的错误仍可见、可修复，而新增提供方编辑的校验不会放宽。配置错误与远端模型是否存在仍然不同：显式指定协议的目录外 ID 可以被接受，由端点决定该 ID 是否存在。标量或文档错误仍尽早失败，因为正是它们会在注册时拒绝整个命名空间；拥有方无法服务的取值则改为按模型报告诊断，因此携带此类取值的已存段仍能完成注册并可修复。不增加 Settings API、存储格式或 session 事件；可配置提供方条目在原有单一字段之外新增一个可选诊断映射，模型列表也新增一个可选逐条目原因。
